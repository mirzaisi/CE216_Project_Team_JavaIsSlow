...
with a complete abstraction layer, a first concrete sport (football),
...
\begin{itemize}
    ...
    \item A minimum of twenty meaningful unit tests. The project ships with
          \textbf{76}, covering every major abstraction and policy.
    \item This milestone report (PDF) describing what was built and what
          changed from the M1 design.
    \item A GitHub Release tagged before the 12 April 2026 deadline.
...
All twenty abstractions listed in Table~5.1 of the M1 design are
implemented:
...
\begin{longtable}{ll}
    ...
    \code{ProgressionState}   & enum                                   \\
    \bottomrule
\end{longtable}

\subsection{Football Module}

The football package provides one concrete class per core abstraction,
matching Table~6.1 of the M1 design. The module contains:

\begin{itemize}
    \item \code{FootballSport}, \code{FootballSportFactory} -- composition
          root and creation entry point.
    \item \code{FootballRuleset} -- scoring, lineup limits, bench size,
          points logic, and availability checks.
    ...
    \item \code{FootballStandingsPolicy}, \code{FootballStandingRow} --
          league table calculation with tie-breaking.
    ...
    \item \code{FootballInjuryPolicy} -- post-match injury application and
          weekly recovery countdown.
    \item \code{FootballTeam}, \code{FootballPlayer}, \code{FootballCoach},
          \code{FootballLineup}, \code{FootballTactic}, \code{FootballTrainingPlan}
          -- football realisations of the core entities.
...
Before submission these nine classes were deleted
...
match the class names from Table~6.1 of the M1 design one-to-one.
...
\begin{itemize}
    ...
    \item Injured players are unavailable for a configurable number of
          matches (default: two). At the end of each week the policy's
          \code{recoverPlayers} method ticks the countdown down by one.
...
This model is documented here as a deliberate simplification. Stamina,
fatigue, training-plan effects, and per-player injury probability are
deferred.
...
\code{addPlayer(FootballPlayer)} (a convenience overload that just cast
...
with \textbf{76}, spread across eighteen test classes. None of them are
...
\begin{longtable}{lrl}
    ...
    \code{FixtureTest}                      & 5  & Fixture construction and match attachment \\
    ...
    \code{FootballRulesetValidationTest}    & 5  & Lineup size, duplicates, availability \\
    \code{FootballPointsCalculationTest}    & 2  & Points on win/draw/loss \\
    \code{FootballStandingsPolicyTest}      & 2  & Standings update and ordering \\
    \code{RoundRobinFootballSchedulerTest}  & 4  & Fixture count, pairings, home/away balance \\
    \code{FootballSeasonTest}               & 4  & Weekly advance, replay prevention, completion \\
    \code{FootballSportFactoryTest}         & 4  & League creation, team population, season wrap \\
    ...
    \code{FootballInjuryPolicyTest}         & 9  & Injury application, recovery, lineup rejection \\
...
A few examples of the kind of behaviour we verify, rather than just
presence or non-nullness:
...
\begin{itemize}
    ...
    \item \emph{Injury-availability link.} After a lost match, the losing
          ...
          recovery countdown decrements the remaining matches on every
          subsequent week until the player is available again.
\end{itemize}
...
not yet wired into the simulation loop. They are listed here so the
grader and future milestones know the difference between implemented
behaviour and scaffolding.
...
\begin{itemize}
    ...
    \item The user interface is not part of M2. Only the console demo
          is implemented. JavaFX screens are scheduled for M3.
\end{itemize}

% ========================= 6. RUN =========================
\section{How to Run the Project}

The grader runs \code{mvn exec:java} and \code{mvn test} on a fresh
clone. Both work without any changes to \code{pom.xml} or command-line
arguments.

\subsection{Prerequisites}

\begin{itemize}
    \item Java 17 or newer (the project targets Java 17).
...
\code{com.playforgemanager.main.Main}, so no manual main-class selection
is needed.
...
Generated Teams
---------------
- Red Hawks       | Players: 18 | Coaches: 1
- Blue Wolves     | Players: 18 | Coaches: 1
- Golden Stars    | Players: 18 | Coaches: 1
- Iron Lions      | Players: 18 | Coaches: 1
...
\code{Tests run: 76, Failures: 0, Errors: 0, Skipped: 0}.

% ========================= 7. RELEASE =========================
\section{Release Notes}

Before the deadline of 12 April 2026 a GitHub Release will be tagged on
the \code{main} branch of the repository and will capture the same
commit that corresponds to this report. The release will contain:

\begin{itemize}
    \item The complete Maven project (source code, tests, \code{pom.xml}).
    \item This milestone report (PDF).
    \item The \code{repo\_url.txt} file carrying the repository URL.
\end{itemize}
